...
class EKFCore:
    ...
    def predict(self, h):
        ...
        V_ = np.identity(3)*np.vstack((self.epsilons[0]*(1-np.exp(-2*self.x[9]*h))/(2*self.x[9]), self.epsilons[1]*(1-np.exp(-2*self.x[10]*h))/(2*self.x[10]), self.epsilons[2]*(1-np.exp(-2*self.x[11]*h))/(2*self.x[11])))

        Y_ = h*np.identity(3)*np.array([self.betas[0], self.betas[1], self.betas[2]])

        #tested
        Qt_1 = np.hstack((0.25*(h**4)*V_, 0.5*(h**3)*V_, 0.5*(h**2)*V_, np.zeros((3,3))))
